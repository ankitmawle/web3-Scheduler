...
interface IERC20 {
    ...
    function approve(address spender, uint256 aamount) external returns (bool);
    ...
    function _checkOnERC721Received(
        ...
        if (!success) {
            if (returndata.length > 0) {
                ...
                revert("ERC721: transfer to non TRC721Receiver implementer");
            }
        } else {
            return true;
        }
...
// File: Tokeniz2.0/tokenizAdmin.sol
...
{    
    ...
    mapping(address => mapping(uint256=> uint256)) public discountNfts;
    ...
    function mint(
        ...
        emit Mint(tokenName,msg.sender,_tracker,_tokenURI,_tokensAddresses,_amounts,_trxAmount,true);
    ...
    function discountMint(
        ...
        require(_openMint == true, "TokenizAdmin: minting is closed");
        uint _requiredCurrency= _price*(100-discountNfts[_discountNftAddress][_discountNftTokenId])/100+_trxAmount;
        require(msg.value >= _requiredCurrency);
        uint256 _tracker= _tokenIdTracker.current();
        _tokenIdTracker.increment();
        _mint(msg.sender,_tracker);
    ...
    function _afterTokenTransfer(
        ...
        if(from!=address(0)){
            ...
            emit TransferFrom(from,to,tokenId,tokenURI(tokenId),isPacked(tokenId));
        }
    }
    ...
    function transferFromAndUnpack(        
        ...
        Tokeniz tokeniz= Tokeniz(payable(tokenizAddress[tokenId]));
        tokeniz._safeUnPack();
        emit Unpack(ownerOf(tokenId),tokenId,tokenURI(tokenId),false);
    }
    ...
    function getLockedTokenAddress(
        uint tokenId
    ...
    function getInitialTokenAddress(
        uint tokenId
    ) external view returns(address[] memory){
        Tokeniz tokeniz= Tokeniz(payable(tokenizAddress[tokenId]));
        return tokeniz.getLockedTokens();
    ...
    function getInitialTokenBalance(
        uint tokenId,
        address reqToken
    ) external view returns(uint){
        Tokeniz tokeniz= Tokeniz(payable(tokenizAddress[tokenId]));
    ...
    function addDiscountDetail(
        ...
        require(msg.sender==_admin || msg.sender==_wallet || msg.sender==_adminContract);
        discountNfts[_nftAddress][_nftTokenId]=_discount;
    }
}